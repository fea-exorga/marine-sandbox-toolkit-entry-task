const BASE_SIZE = 32;
    ...
    function place(type, emoji, x, y) {
        ...
        state.assets_placed.push({ id, type, emoji, x: Math.round(x), y: Math.round(y) });
        render(id);
        select(id);
    ...
    function render(id) {
        ...
        selected_asset.innerHTML = `
            ...
            <button class="delete" onclick="removeAsset('${id}',event)">✕</button>
    ...
    function removeAsset(id, e) {
        ...
        e?.stopPropagation();
        document.getElementById(id)?.remove();
        state.assets_placed = state.assets_placed.filter(a => a.id !== id);
    ...
    function exportJSON() {
        ...
        const data = {
            ...
            assets:   state.assets_placed.map(({ id, type, emoji, x, y }) => ({ id, type, emoji, x, y }))
        };
    ...
    document.addEventListener('keydown', e => {
        if (e.key === 'Delete' && state.selectedId && e.target.tagName !== 'INPUT') removeAsset(state.selectedId);
